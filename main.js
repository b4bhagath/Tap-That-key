...
function onKeyDown(event) {
    ...
    if (keyData[event.key]) {
        var maxPoint = new Point(view.size.width, view.size.height); // Checks the size of the device
        var randomPoint = Point.random();
        var point = maxPoint * randomPoint;
        var newCircle = new Path.Circle(point, 500); // Create a random circles
        newCircle.fillColor = keyData[event.key].color;
        circles.push(newCircle); // push it to the array
    }


}

//Loop through the circle and scale it and animate the color using hue
